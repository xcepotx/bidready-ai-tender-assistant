...
HOOK = Path("apps/web/src/hooks/useTenderPostMvpUpdates.js")
...
def find_async_function(text, name):
    # Flexible: accept both correct "  async function" and accidentally unindented "async function".
    ...
    # Remove at most one blank line after the function; preserve next token.
    if text[end:end + 2] == "\n\n":
        end += 1
...
def main():
    ...
    app_backup = APP.with_suffix(".jsx.bak.extract-post-mvp-update-handlers-hook")
...
'''
    ...
    if 'import { useTenderPostMvpUpdates } from "./hooks/useTenderPostMvpUpdates.js";' not in new_text:
...
'''
    ...
    if "useTenderPostMvpUpdates({" not in new_text:
        insert_at = parse_hook_call_end(new_text, "useTenderPostMvpGenerators")
