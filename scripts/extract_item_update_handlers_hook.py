...
HOOK = Path("apps/web/src/hooks/useTenderItemUpdates.js")
...
def find_async_function(text, name):
    ...
    # Preserve next token exactly. Only remove one trailing blank line if present.
    if text[end:end+2] == "\n\n":
        end += 1
...
def main():
    ...
    app_backup = APP.with_suffix(".jsx.bak.extract-item-update-handlers-hook")
...
'''
    ...
    if 'import { useTenderItemUpdates } from "./hooks/useTenderItemUpdates.js";' not in new_text:
...
'''
    ...
    if "useTenderItemUpdates({" not in new_text:
        insert_at = parse_hook_call_end(new_text, "useTenderSimpleUpdates")
