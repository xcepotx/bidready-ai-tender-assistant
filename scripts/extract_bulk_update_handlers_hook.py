...
HOOK = Path("apps/web/src/hooks/useTenderBulkUpdates.js")
...
def find_async_function(text, name):
    ...
    # Remove at most one blank line after function; preserve next token.
    if text[end:end + 2] == "\n\n":
        end += 1
...
def main():
    ...
    app_backup = APP.with_suffix(".jsx.bak.extract-bulk-update-handlers-hook")
...
'''
    ...
    if 'import { useTenderBulkUpdates } from "./hooks/useTenderBulkUpdates.js";' not in new_text:
...
'''
    ...
    if "useTenderBulkUpdates({" not in new_text:
        insert_at = parse_hook_call_end(new_text, "useTenderItemUpdates")
